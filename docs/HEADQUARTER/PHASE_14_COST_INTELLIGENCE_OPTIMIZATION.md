...
| WHICH budget policies govern a piece of work | `#governingBudgetScopes` — the deployment baseline, plus the task's canonical missions and projects (`#canonicalTaskScopes`, off `hq_mission_plan_items.mission_id` with `hq_missions` joined LEFT for the project), plus every project the task's mission(s) have EVER been bound to (`#durableTaskProjectScopes`, off the append-only `hq_mission_events`), plus every scope the task has ALREADY SPENT UNDER (`#recordedScopesForTask`, off the append-only attribution on `hq_intel_cost_entries`), plus `#taskBoundProvider`, all through `#db` | which ceilings a decision write is enforced against at all | canonical **since the Wave 5 correction of HIGH B-1**; it was previously an optional `budgetScope` ARGUMENT taken verbatim, so the caller chose the policy governing its own write. **This row named only the canonical term until round thirteen (Low 7)**, although the other two are load-bearing elsewhere on this page: the recorded-spend union is round four's High H2 and the durable-project term is round seven's High NEW-4, and each is the sole reason a ceiling keeps binding under a different attack. Both are now pinned by execution in `budget-scope-identity.test.ts`. |
...
| the mission and project a decision or cost entry is ATTRIBUTED to | `#canonicalTaskScopes` — the mission from `hq_mission_plan_items.mission_id`, with `hq_missions` joined LEFT for the project (an INNER join here was round thirteen's High 3), through `#db` | whose ceiling the spend counts against | canonical **since HIGH B-2**; both were caller parameters written verbatim, and neither was checked for existence. |
...
## Wave 5 correction round THIRTEEN: a budget ceiling nullified through a row IDENTITY

### HIGH 3 — a SIXTH route, one raw `UPDATE`, no DDL, silent at both integrity depths

**Reproduced at the merged head `237fc76`**, 12 of 12 fresh identities:

| Primitive | What it does | Measured |
|---|---|---|
| `UPDATE hq_missions SET id = …` | makes `#canonicalTaskScopes`' inner `JOIN hq_missions m ON m.id = p.mission_id` match nothing | `governedBy` `['deployment','mission']` → `['deployment']`; `permittedTiers` `['deterministic_local']` → all five; `budgetDecision` `blocked` → `within_ceiling`; a refused `critical_review` write ACCEPTED; `hq_missions` row count unchanged; `structuralIntegrity` clean |
| `UPDATE op_tasks SET id = …` | leaves every `hq_mission_plan_items.task_id` naming a task that no longer exists | the same, and the PROJECT half empties too |

`hq_missions` carried `no_erase` (BEFORE DELETE) and `no_replace` (BEFORE
INSERT) and nothing that guarded an UPDATE of `id` — although its declaration in
`ENGINE_IMMUTABLE_TABLES` said "what is write-once is the row's EXISTENCE and
its identity" and the `no_replace` trigger's own message said "hq_missions
identity is write-once". `op_tasks` carried no guards at all. The class is the
same one this round is otherwise about: the append-only trio standing in for
"every way a row's identity can change", and an inner JOIN standing in for
"canonical membership".

**What changed, in two halves that are not substitutes for each other.**

1. **The read no longer depends on the joined identity.** `#canonicalTaskScopes`
   takes the mission from `hq_mission_plan_items.mission_id` — the plan item's
   own column, write-once by its own `no_remission` guard — and joins
   `hq_missions` LEFT, for the project only. A mission row that cannot be found
   no longer removes the mission scope.
2. **The write is refused.** `trg_hq_missions_no_reidentify` (declared as a
   `secondaryGuards` entry on the ledger it belongs to) and
   `trg_op_tasks_no_reidentify` (declared in `WRITE_ONCE_IDENTITY_TABLES`, a new
   second class for tables that are legitimately updated and are therefore not
   append-only ledgers). Both are censused by `missingImmutabilityGuards`, so
   their absence is `append_only_guard_missing` rather than silence, and the
   live-schema pin in `reliability-durability.test.ts` was EXTENDED — the live
   trigger set still equals the union of the declarations, that union now has
   two terms — rather than relaxed.

Pinned in `budget-scope-identity.test.ts`, including the read-side half proven
with the guard DROPPED, since a guard is three statements.

### MEDIUM 6 and MEDIUM 7 — two load-bearing defences that nothing pinned

Round four's recorded-attribution union has two halves, and removing either left
the whole suite green at `237fc76` (3425 tests), with no test in the package
referencing `entry.missionIds`, `#recordedScopesForTask` or `spentUnder` at all:

- **Medium 7**, the `spentUnder` union in `#governingBudgetScopes`: without it, a
  task whose canonical link has been broken by the disclosed count-preserving
  rewrite loses the exhausted mission from the governing set entirely.
- **Medium 6**, the `entry.missionIds` term in the mission MEASUREMENT: without
  it, the scope stays in the governing set and its measurement comes back empty,
  so the Founder is shown `within_ceiling` on a ceiling that is exhausted — the
  report and the enforcement disagreeing about the same mission.

Both are now pinned by execution in `budget-scope-identity.test.ts`, in the one
scenario that can see them: a task with its OWN recorded spend, after the plan
