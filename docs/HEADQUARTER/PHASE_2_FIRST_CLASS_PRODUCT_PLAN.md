...
- 3D shows real state or nothing — held, and tightened: liveness is computed in
  ...
  room the state document did not.
- The existing invariants (§5) all survive: the emitted HTML still carries no form,
  button or inline handler; every page fetch is allow-listed; every string reaches the
  DOM through `textContent`.

**One deviation from this plan, decided during implementation and recorded here rather
than escalated** (routine structural choice, per `CLAUDE.md`): the runtime landed as
`packages/headquarter/src/client/` with its own export subpath
(`@factoryos/headquarter/client`), *not* as a separate `packages/hq-client` workspace.
A separate package would have been circular from the first commit — `ui/render.ts` and
`ui/site.ts` must import the immersive page body to emit it, while the client must
import `live/snapshot.ts` and `ui/components.ts` — and a circular workspace dependency
is a worse boundary than a clear module with a declared subpath. Extracting it later is
mechanical: the module imports exactly three things from the rest of the package
(`live/snapshot`, `live/control-api` for the route constants, `ui/components` for
`escapeHtml`/`section`), and nothing imports it except `ui/render.ts`, `ui/site.ts`,
`ui/theme.ts` and the one new `live/control-api.ts` route.

**Also decided during implementation:** the 3D shell is written directly against WebGL
rather than Three.js / React Three Fiber. The building is procedural — boxes, a floor
grid, glass facades and light columns, ~1.2k triangles — so a scene graph would have
bought nothing, and taking the dependency would have meant either a CDN `<script>` (an
untracked external asset on a page that renders canonical company state, and a
third-party origin on the one document that must stay same-origin for its session
cookie) or a bundler in a package whose architecture is pure functions from data to
HTML. The whole shell — shaders, renderer and building — is ~29 kB gzipped, smaller
than a 3D library's minified core, which is also why no lazy-loading split was needed.
If a later phase needs skinned meshes, glTF or post-processing, that is the point to
take the dependency.
