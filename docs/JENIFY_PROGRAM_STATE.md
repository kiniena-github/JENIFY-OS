...
| W1-A6 | Owner daily brief (in-app): what happened / needs attention | A5 | services/brief.ts; /api/brief; web BriefCard | **DONE (server)** — happened+attention digest, financial-masked, severity-sorted; 2 tests. Web card via agent |
| W1-A7 | Mobile viewport regression harness (Founder-approved) | A2 | packages/web vitest+jsdom | **IN PROGRESS** — agent bootstrapping the web test harness + BriefCard |
| W1-G  | AI safe-action substrate (preview→confirm→execute, typed action registry, risk gating) | A5 | services/aiActions.ts; routes/assistant.ts | **DONE** — full pipeline; only draft-risk executes, post/destructive registered-but-gated; execute needs preview token bound to params; every step audited; 7 tests |
| W1-B2 | Offline O2 #2 — DELIVERY CONFIRMATION | A3 | services/syncops.ts delivery.confirm handler | **DONE** — offline proof-of-delivery replays at-most-once through markDelivered; server-authority hardened |
