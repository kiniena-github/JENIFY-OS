...
describe('the safe-mode verdict survives a restart, because it is RECORDED', () => {
  ...
  it('carries only BLOCKING findings forward, and only from a safe-mode verdict', () => {
    ...
    expect(forged.safeMode).toBe(false);
...
describe('the enforcement declarations are frozen, not merely typed readonly', () => {
  ...
  it('sees the guards of a table that is NOT append-only as a whole', () => {
    const fx = fileFixture();
    try {
      // `hq_mission_plan_items` is legitimately updated, so it carries
      // `no_replace` / `no_relink` / `no_respec` instead of the trio — and
      // while the table was unlisted the census could not see any of them.
      // `trg_hq_mission_plan_items_no_replace` is what stops an
      // `INSERT OR REPLACE` rewriting a plan item's task binding.
