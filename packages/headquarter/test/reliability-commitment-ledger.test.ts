...
describe('HQ’s own commitment ledger is checked against itself', () => {
  ...
  /**
   * RE-PRICED at round seven, downwards, and measured rather than carried
   * forward.
   *
   ...
   * Founder assessment it was aiming to pass. It no longer buys even that.
   *
   * The reason is High 1's fix, not a new check: HQ's committed mark used to be
   * gated on the ledger having a `sqlite_sequence` row, so deleting that row
   * meant HQ saw nothing to commit about its own commitment ledger and appended
   * nothing during the assessment. The mark is now read from the ledger's own
   * rows, so the very first thing that assessment does is append a checkpoint,
   * which re-creates the high-water mark from the rowid the forged row still
   * carries, and the identity breaks inside the same process. The forger cannot
   * compose around it: the replacement row's own mark is exactly what the
   * variant above (`repairSequence: true`) has to commit, and committing it is
   * what makes the collapse visible instead.
   */
  it('costs the cheaper one-statement repair every process, the first included', () => {
    const fx = fileFixture();
    try {
      warm(fx);
      fx.db.close();
      ...
      // The boot itself still reads clean: there is no high-water mark to
      // compare the surviving row against, which is the whole of what the one
      // statement buys.
      const bought = fx.reopen('the-boot-it-buys');
      expect(bought.ops.hqReliabilityPosture().integrity.safeMode).toBe(false);
      // The Founder assessment it was aiming to pass does NOT pass. That
      // assessment is also the act that appends the next commitment, and
      // appending it re-creates the mark the forger deleted.
      ...
      expect(findings(boughtAssessment.data.observations)).toContain('append_only_guard_missing');
      expect(bought.ops.releaseKillSwitch('global', 'founder').ok).toBe(false);
      bought.db.close();

      // And it stays broken, in every process afterwards.
