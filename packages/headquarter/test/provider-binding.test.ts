...
describe('the provider write mechanism is not reachable around the authority gate', () => {
  ...
  it('exposes only its allowlisted public surface, so nothing new becomes patchable', () => {
    ...
    const PUBLIC_SURFACE = new Set([
      ...
      'complete',
      'fail',
      // `releaseClaim` arrives with the #221/#224 dispatch lane and is in the
      // same category, checked against the criterion rather than waved through:
      // it WRITES (evidence, then a transition back to `needs_approval` and a
      // cleared claim), and the queue itself resolves the authority — the first
      // statement inside its transaction is `#assertFence(taskId, workerId,
      // fence)`. Only the current claim holder has that fence, so a caller can
      // release its OWN claim and nothing else. It grants nothing: the task
      // returns to a gated state rather than a runnable one.
      'releaseClaim',
      ...
      'selectClaimable',
      'listWorkerProviders',
      // Reads added with the #224 dispatch lane. Each is a closure or wrapper
      // over a `#private` resolver that the enforcement path calls DIRECTLY, so
      // patching the public name changes what the patcher sees and nothing
      // about what `claim`/`start` enforce — the distinction that removed `get`
      // from this category is precisely that enforcement dispatched through it.
      //
      // `assignabilityProblem` answers the question `claim` asks, without
      // asserting anything: `claim` still calls `assertAssignable` inside its
      // own write path. `providerOf` reads the declared worker→provider map
      // through `#providerOf`, which `#selectClaimableInternal` also calls
      // directly. `approvalFor` reads an approval record through
      // `#getApprovalRecord`; `#validateTaskApproval` does not go through it.
