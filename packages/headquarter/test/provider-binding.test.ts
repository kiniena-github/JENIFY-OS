...
function bindingFixture(): Fixture {
  ...
  // Provider identity is DECLARED, never inferred from a vendor string.
  fixture.ops.queue.workerProviders.declare('claude-worker', 'CLAUDE', 'founder');
  fixture.ops.queue.workerProviders.declare('codex-worker', 'CODEX', 'founder');
  return fixture;
}

/** Create a CLAUDE-routed order and get it past the Founder gate into `queued`. */
...
describe('deny by default: an undeclared worker holds no provider', () => {
  ...
  it('refuses everyone when the binding itself is malformed', () => {
    ...
    fx.ops.queue.workerProviders.revoke('claude-worker');
  ...
  it('still requires the capability grant: binding removes candidates, never adds one', () => {
    ...
    fx.ops.queue.workerProviders.declare('jules', 'CLAUDE', 'founder');
...
describe('the binding holds at start, not only at claim', () => {
  it('refuses to start when the worker no longer executes as the bound provider', () => {
    ...
    fx.ops.queue.workerProviders.declare('claude-worker', 'CODEX', 'founder');
