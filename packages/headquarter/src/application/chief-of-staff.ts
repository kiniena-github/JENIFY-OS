 ...
 * gates, never invented. `worker_claim` is the one non-Founder authority:
 * a queued task is claimable only by a registered worker holding the
 * capability under the directory grant.
 */
...
/** The canonical tables an attention item may reference. */
...
export function orderAttentionItems(items: readonly AttentionItem[]): AttentionItem[] {
...
function itemId(kind: AttentionKind, reason: AttentionReason, sourceId: string): string {
 ...
 */
export function deriveFounderInbox(facts: CommandFacts): AttentionItem[] {
  ...
  // approval: decision = 'pending' → the Founder gate holds the task.
  for (const approval of facts.approvals) {
    if (approval.decision === 'pending') {
      items.push({
        id: itemId('approval', 'approval_pending', approval.id),
        kind: 'approval',
        reason: 'approval_pending',
        source: { table: 'hq_approvals', id: approval.id },
        entities: [
          { kind: 'approval', id: approval.id },
          ...(approval.taskId ? [{ kind: 'task' as const, id: approval.taskId }] : []),
        ],
        summary: `Approval ${approval.id} is pending on task ${approval.taskId ? taskLabel(approval.taskId) : '(none)'} (${approval.riskClass}), requested by ${approval.requestedBy}.`,
        since: approval.requestedAt,
        staleness: 'current',
        requiredAuthority: 'approval_authority',
        provenance: 'hq_approvals.decision = pending',
        founderOnly: false,
      });
      continue;
    }
  ...
  for (const pair of facts.contradictions) {
    ...
    items.push({
      ...
      entities: [
        ...
        { kind: pair.entityKind === 'mission' ? 'mission' : pair.entityKind === 'task' ? 'task' : pair.entityKind === 'worker' ? 'worker' : pair.entityKind === 'capability' ? 'capability' : pair.entityKind === 'project' ? 'project' : 'truth', id: pair.entityId },
      ],
  ...
  for (const task of facts.tasks) {
    if (task.status === 'blocked' || task.status === 'review_failed') {
      const reason: AttentionReason = task.status === 'blocked' ? 'task_blocked' : 'task_review_failed';
      items.push({
  ...
  for (const record of facts.truth) {
    ...
    items.push({
      ...
      entities: [
        ...
        { kind: record.entityKind === 'mission' ? 'mission' : record.entityKind === 'task' ? 'task' : record.entityKind === 'worker' ? 'worker' : record.entityKind === 'capability' ? 'capability' : record.entityKind === 'project' ? 'project' : 'truth', id: record.entityId },
      ],
...
export function countByKind(items: readonly AttentionItem[]): Record<AttentionKind, number> {
...
const ATTENTION_TO_RECOMMENDATION: Readonly<
  Record<AttentionReason, { kind: RecommendationKind; actPath: string; limitations: string[] }>
> = {
  approval_pending: {
...
function authorityFor(item: AttentionItem): RequiredAuthority {
  return item.requiredAuthority;
}

/** One recommendation per inbox item — derived, deterministic, never persisted, never executable. */
export function deriveRecommendations(items: readonly AttentionItem[]): Recommendation[] {
  return items.map((item) => {
    ...
    return {
      ...
      requiredAuthority: authorityFor(item),
...
export interface BlockedView {
  ...
  heldForApproval: BoundedList<{ taskId: string; title: string | null; approvalId: string | null; requestedAt: string | null }>;
  killSwitches: KillSwitchFact[];
...
export function deriveBlocked(facts: CommandFacts, limit = BRIEFING_SECTION_LIMIT): BlockedView {
  const approvalByTask = new Map<string, ApprovalFact>();
  for (const approval of facts.approvals) if (approval.decision === 'pending' && approval.taskId) approvalByTask.set(approval.taskId, approval);
  ...
  const held = facts.tasks
    ...
    .map((task) => {
      ...
      return { taskId: task.id, title: task.title, approvalId: approval?.id ?? null, requestedAt: approval?.requestedAt ?? null };
    });
...
export function truthRefStaleness(record: TruthFact): Staleness {
  return truthStaleness(record.subjectDrift);
}

export function deriveVerified(facts: CommandFacts, limit = BRIEFING_SECTION_LIMIT): VerifiedView {
  const established = facts.truth.filter((record) => record.state === 'verified' || record.state === 'accepted');
...
export function deriveUnknown(facts: CommandFacts, limit = BRIEFING_SECTION_LIMIT): UnknownView {
  ...
  const inconclusive = facts.truth
    .filter((t) => t.lifecycle === 'current' && t.verification === 'inconclusive')
    .map((t) => ({ id: t.id, entityKind: t.entityKind, entityId: t.entityId, founderOnly: t.privacy === 'founder_only' }));
  const bare = facts.truth
    .filter((t) => t.lifecycle === 'current' && t.evidenceRefs.length === 0)
    .map((t) => ({ id: t.id, entityKind: t.entityKind, entityId: t.entityId, state: t.state, founderOnly: t.privacy === 'founder_only' }));
...
export interface SafeAct {
  act: SafeActKind;
  nature: 'read' | 'record';
  /** True when every stated precondition holds NOW; otherwise `blockers` names what does not. */
  safe: boolean;
  blockers: string[];
  requiredAuthority: RequiredAuthority;
  targets: EntityRef[];
...
export function deriveSafeNext(facts: CommandFacts, limit = BRIEFING_SECTION_LIMIT): SafeNextView {
  const acts: SafeAct[] = [];
  ...
  for (const mission of facts.missions) {
    ...
    if (mission.status === 'blocked') blockers.push('mission is blocked (a Founder stop)');
    if (mission.status === 'ready_review' || mission.status === 'verified') blockers.push(`mission is ${mission.status} (past building)`);
    if (globalStop) blockers.push('global kill switch engaged');
    if (orchestrateStop) blockers.push('orchestrate-scope kill switch engaged');
    if (!orchestrateRow) blockers.push('hq.mission_orchestrate is not registered');
    else if (!orchestrateRow.enabled) blockers.push('hq.mission_orchestrate is disabled');
    for (const scope of mission.engagedSpecScopes) blockers.push(`kill switch engaged for spec capability ${scope}`);
    for (const capabilityId of mission.specCapabilitiesUnavailable) blockers.push(`spec capability ${capabilityId} is not registered and enabled`);
    acts.push({
      ...
      note:
        `${specified.length} specified, unlinked work item(s). A preview is a pure read and is always safe; apply creates canonical tasks and takes the orchestrate gate, the mission gate and step-up` +
        (blockers.length > 0 ? ', and would currently refuse: ' + blockers.join('; ') : '') +
        '.',
    });
  }
  acts.push({
    ...
    blockers: facts.stores.briefs ? [] : ['the brief ledger is absent on this handle'],
...
export function deriveChanged(input: {
  ...
  return {
    ...
    note: input.since
      ? `Canonical hq_events rows appended after brief ${input.since.id} (event seq > ${input.since.watermark.eventSeq}) and op_evidence entries after evidence seq ${input.since.watermark.evidenceSeq}, by kind. A delta over the append-only record; nothing is summarised.`
      : 'No brief has been issued, so there is no watermark to measure from: this section carries the newest canonical events and evidence kinds overall, and is NOT a delta.',
  };
}
...
export function deriveDepartments(facts: CommandFacts, inbox: readonly AttentionItem[]): DepartmentProjection[] {
  ...
  return [
    ...
    },
    {
      ...
      metrics: [
        ...
        { label: 'Authority refusals recorded (all time)', value: refusals },
      ],
      attention: kinds('incident', 'contradiction', 'risk'),
      note: 'Posture facts from the canonical record: stops, disputes, risk-classed external actions and the refusals the evidence chain kept. Secret presence never crosses here.',
    },
    {
      department: 'research',
      basis: 'not_recorded',
...
export interface CommandCenterSnapshotView {
  attention: { total: number; byKind: Record<AttentionKind, number>; withheldFounderOnly: number; items: AttentionItem[] };
  unknown: { total: number };
  blocked: { total: number };
  recommendations: { total: number };
  briefs: { total: number; latest: BriefView | null };
