 ...
 * `elidedCommitmentLedgerRows`: `sqlite_sequence` is writable and no trigger can
 * guard it, so an explicit `seq` on the replacement row plus one
 * `UPDATE sqlite_sequence` stays silent. That is a cost, not a boundary, and it
 * is the same residual class as zeroing `PRAGMA user_version`: HQ holds no key a
 * foreign writer does not also have, and every fact this ledger rests on lives
 * in the same file the attacker is already writing.
 */
 ...
 * merge). `truncatedImmutableLedgers` below asserts `MAX(rowid) >= high-water`
 * over EVERY declared AUTOINCREMENT ledger; the identity asserted here is
 * strictly stronger for this one table, because it also counts the rows. A wipe
 * that INSERTS a replacement carrying the old greatest rowid satisfies the
 * general relation and fails this one. Neither check subsumes the other, so both
 * stand: the general rule is what closes the identical attack on
 * `hq_reliability_run_events` and every other declared ledger, and this one is
 * what closes the replacement-row variant on the commitment ledger itself.
 *
