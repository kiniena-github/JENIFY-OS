 ...
 *    are.
 *
 * ## The carrier class, enumerated rather than patched one at a time
 *
 * Round fifteen's Medium 3 was a FOURTH instance of the same shape, so the class
 * is written down here instead of a fifth being waited for. This walk descends
 * into exactly five carriers, and each one carries two kinds of publishable
 * text: the CONTENT it holds, and the LABEL it holds that content under. A
 * carrier that reads only one of the two is a hole, because `JSON.stringify`
 * emits both.
 *
 * | Carrier | Content | Label |
 * |---|---|---|
 * | array | every element, walked | index — generated by the engine, never data |
 * | `Map` | every value, walked | every key, walked |
 * | `Set` | every member, walked | none |
 * | `toJSON()` | the projection, walked | the literal `.toJSON()`, never data |
 * | plain object | every value, walked | **every key, visited** |
 *
 * The bolded cell is the one that was missing. Only three of the five carriers
 * have a label that can be data at all, and all three now read it. A future
 * carrier added to this function must fill in both columns or say in its own
 * branch why its label cannot carry caller text.
 *
 * **How far the hole reached, recorded rather than rounded either way.** At unit
 * level and through the facade it was fully open: `proposeAction` accepted a
 * credential written as a payload key and stored it verbatim in
 * `hq_action_intents`, a ledger whose `no_erase` guard means it could not then
 * be removed without DDL. On the SNAPSHOT it was narrower. The artifact has two
 * object keys that come from data rather than from code — `workerProviders` and
 * `workerMembers` in `live/snapshot.ts`, keyed by worker and member id — and
 * both of those ids are separately scanned as VALUES elsewhere in the same
 * payload, so a credential-shaped id was refused at that boundary anyway. The
 * finding was therefore reachable through the write path and, as far as this
 * correction could establish, not through the published artifact. That
 * distinction stops mattering now: the walk reads the key, so a future
 * `Record<string, …>` keyed by caller text with no matching value occurrence is
 * covered by construction rather than by an argument about which keys exist
 * today.
 *
...
function walk(
  ...
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    const at = `${path}.${key}`;
    // The property NAME is offered to the VALUE rule as a string in its own
    // right, and not only to the key rule as a name (Wave 5 correction round
    // fifteen, Medium 3). Without this line the plain-object branch was the one
    // carrier of the five that read what it carried and not what it was LABELLED
    // WITH, and `JSON.stringify` publishes both: a credential written as a
    // property name — `{"ghp_…": 1}` — walked straight through
    // `assertBrowserSafe` while the identical string one field over was refused.
    // Executed end to end through `proposeAction` before the fix: the value form
    // was refused `String matches a known credential shape`, the KEY form was
    // accepted and stored verbatim in `hq_action_intents`, whose `no_erase`
    // guard means it could not then be removed without DDL.
    //
    // The `Map` branch above already did exactly this, and that asymmetry is
    // what shows the omission was an oversight rather than a trade: a `Map` is
    // an object whose field names happen to be data, and a plain object is an
    // object whose field names may equally be data — every `Record<string, …>`
    // the control plane composes is keyed by something a caller chose.
    visit(key, `${at}.key`);
    visit(child, at, key);
    walk(child, at, visit, seen, depth + 1);
  }
}
