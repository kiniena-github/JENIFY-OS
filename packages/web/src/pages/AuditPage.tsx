...
export default function AuditPage() {
  ...
  const todayCount = (data?.rows ?? []).filter((r) => r.createdAt.startsWith(today)).length;

  return (
    <div>
      <div className="cards">
        <StatCard label={t('audit.total', 'Events (filtered)')} value={data?.count ?? '—'} />
        <StatCard label={t('audit.today', 'On latest page today')} value={todayCount} />
      </div>
            ...
            <tbody>
              {(data?.rows ?? []).map((r) => (
                <tr key={r.id} className="clickable" onClick={() => setViewing(r)}>
                  ...
                  <td>
                    <span className="badge badge-gray">{r.module}</span>
                  </td>
      ...
      {viewing ? (
        <Modal title={`${viewing.action} — ${viewing.reference ?? ''}`} onClose={() => setViewing(null)} wide>
